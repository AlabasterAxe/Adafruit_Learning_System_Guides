...
GLOBAL_VAR int DISPLAY_SIZE
    GLOBAL_INIT(240);  // Start with assuming a 240x240 display
GLOBAL_VAR int DISPLAY_X_OFFSET
    GLOBAL_INIT(0);  // Used with rectangular screens
GLOBAL_VAR int DISPLAY_Y_OFFSET
    GLOBAL_INIT(0);  // Used with rectangular screens
...
GLOBAL_VAR uint8_t eyelidIndex
    GLOBAL_INIT(0x00);  // From table: learn.adafruit.com/assets/61921
GLOBAL_VAR uint16_t eyelidColor
    GLOBAL_INIT(0x0000);  // Expand eyelidIndex to 16-bit
...
GLOBAL_VAR int8_t blinkPin
    GLOBAL_INIT(-1);  // Manual both-eyes blink pin (-1 = none)
...
eyeStruct eye[NUM_EYES] = {
    ...
    {"right", &ARCADA_TFT_SPI, ARCADA_TFT_CS, ARCADA_TFT_DC, ARCADA_TFT_RST,
     -1},
    {"left", &ARCADA_LEFTTFT_SPI, ARCADA_LEFTTFT_CS, ARCADA_LEFTTFT_DC,
     ARCADA_LEFTTFT_RST, -1}};
#else
...
extern ImageReturnCode loadEyelid(char *filename, uint8_t *minArray,
                                  uint8_t *maxArray, uint8_t init,
                                  uint32_t maxRam);
extern ImageReturnCode loadTexture(char *filename, uint16_t **data,
