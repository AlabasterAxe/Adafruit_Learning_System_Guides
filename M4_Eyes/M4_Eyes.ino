...
int iPupilFactor = 42;
uint32_t boopSum = 0, boopSumFiltered = 0;
bool booped = false;
int fixate = 7;
...
#define DMA_TIMEOUT \
  (uint32_t)((DISPLAY_SIZE * 16 * 4000) / (DISPLAY_FREQ / 1000))
...
// Initialize displays
  ...
  yield();
  if (arcada.drawBMP((char *)"/splash.bmp", 0, 0, (eye[0].display)) ==
      IMAGE_SUCCESS) {
  ...
  for (e = 0; e < NUM_EYES; e++) {
    ...
    (ARCADA_TFT_HEIGHT != 128)  // 160x128 is ST7735 which isn't able to deal
    ...
    eye[e].dptr = eye[e].dma.addDescriptor(NULL, NULL, 42, DMA_BEAT_SIZE_BYTE,
                                           false, false);
    ...
    for (int i = 0; i < 2; i++) {  // For each of 2 scanlines...
      for (int j = 0; j < NUM_DESCRIPTORS;
           j++) {  // For each descriptor on scanline...
        eye[e].column[i].descriptor[j].BTCTRL.bit.VALID = true;
        eye[e].column[i].descriptor[j].BTCTRL.bit.EVOSEL =
            DMA_EVENT_OUTPUT_DISABLE;
        eye[e].column[i].descriptor[j].BTCTRL.bit.BLOCKACT =
        ...
        eye[e].column[i].descriptor[j].BTCTRL.bit.STEPSIZE =
    ...
    eye[e].iris.startAngle =
        (e & 1) ? 512 : 0;  // Rotate alternate eyes 180 degrees
  ...
  for (e = 0; e < NUM_EYES; e++) {  // For each eye...
    ...
    if ((!e) || (e2 >= e)) {  // If first eye, or no match found...
      ...
      if ((eye[e].iris.filename == NULL) ||
          (loadTexture(eye[e].iris.filename, &eye[e].iris.data,
    ...
    if ((!e) || (e2 >= e)) {  // If first eye, or no match found...
      ...
      if ((eye[e].sclera.filename == NULL) ||
          (loadTexture(eye[e].sclera.filename, &eye[e].sclera.data,
  ...
  status = loadEyelid(
      upperEyelidFilename ? upperEyelidFilename : (char *)"upper.bmp",
      upperClosed, upperOpen, DISPLAY_SIZE - 1, maxRam);

  status = loadEyelid(
      lowerEyelidFilename ? lowerEyelidFilename : (char *)"lower.bmp",
...
void loop() {
  ...
  if (!eye[eyeNum].column_ready) {
    if (!x) {  // If it's the first column...
      ...
      if (eyeInMotion) {                  // Currently moving?
        if (dt >= eyeMoveDuration) {      // Time up?  Destination reached.
          ...
          float e =
              (float)dt / float(eyeMoveDuration);  // 0.0 to 1.0 during move
          e = 3 * e * e -
              2 * e * e * e;  // Easing function: 3*e^2-2*e^3 0.0 to 1.0
        ...
        if (dt > eyeMoveDuration) {  // Time up?  Begin new move.
          float r = (float)mapDiameter -
                    (float)DISPLAY_SIZE * M_PI_2;  // radius of motion
          r *= 0.6;
          eyeNewX = random(-r, r);
      ...
      if (eyeNum & 1)
        eyeX += fixate;  // Eyes converge slightly toward center
      else
        eyeX -= fixate;
      ...
      if (tracking) {
        ...
        int ix = (int)map2screen(mapRadius - eye[eyeNum].eyeX) +
                 (DISPLAY_SIZE / 2),  // Pupil position
            iy = (int)map2screen(mapRadius - eye[eyeNum].eyeY) +
                 (DISPLAY_SIZE / 2);  // on screen
      ...
      eye[eyeNum].upperLidFactor =
          (eye[eyeNum].upperLidFactor * 0.6) + (uq * 0.4);
      eye[eyeNum].lowerLidFactor =
          (eye[eyeNum].lowerLidFactor * 0.6) + (lq * 0.4);
      ...
      if (eye[eyeNum].blink.state) {  // Eye currently blinking?
        ...
        if ((t - eye[eyeNum].blink.startTime) >= eye[eyeNum].blink.duration) {
          ...
          eye[eyeNum].blinkFactor = (float)(t - eye[eyeNum].blink.startTime) /
                                    (float)eye[eyeNum].blink.duration;
          if (eye[eyeNum].blink.state == DEBLINK)
            eye[eyeNum].blinkFactor = 1.0 - eye[eyeNum].blinkFactor;
        }
      }
      ...
      if (eye[eyeNum].iris.iSpin) {
        // Spin works in fixed amount per frame (eyes may lose sync, but "wagon
        // wheel" tricks work)
        eye[eyeNum].iris.angle += eye[eyeNum].iris.iSpin;
      } else {
        // Keep consistent timing in spin animation (eyes stay in sync, no
        // "wagon wheel" effects)
        eye[eyeNum].iris.angle = (int)((float)eye[eyeNum].iris.startAngle +
                                       eye[eyeNum].iris.spin * mins + 0.5);
      }
      if (eye[eyeNum].sclera.iSpin) {
        eye[eyeNum].sclera.angle += eye[eyeNum].sclera.iSpin;
      } else {
        eye[eyeNum].sclera.angle = (int)((float)eye[eyeNum].sclera.startAngle +
                                         eye[eyeNum].sclera.spin * mins + 0.5);
      }
    ...
    float upperLidFactor =
              (1.0 - eye[eyeNum].blinkFactor) * eye[eyeNum].upperLidFactor,
          lowerLidFactor =
              (1.0 - eye[eyeNum].blinkFactor) * eye[eyeNum].lowerLidFactor;
    iPupilFactor = (int)((float)eye[eyeNum].iris.height * 256 *
                         (1.0 / eye[eyeNum].pupilFactor));

    int y1, y2;
    int lidColumn = (eyeNum & 1) ? (DISPLAY_SIZE - 1 - x)
                                 : x;  // Reverse eyelid columns for left eye
    ...
    if (upperOpen[lidColumn] == 255) {
      ...
      y1 = lowerClosed[lidColumn] +
           (int)(0.5 + lowerLidFactor * (float)((int)lowerOpen[lidColumn] -
                                                (int)lowerClosed[lidColumn]));
      y2 = upperClosed[lidColumn] +
           (int)(0.5 + upperLidFactor * (float)((int)upperOpen[lidColumn] -
                                                (int)upperClosed[lidColumn]));
      if (y1 > DISPLAY_SIZE - 1)
        y1 = DISPLAY_SIZE - 1;  // Clip results in case lidfactor
      else if (y1 < 0)
        y1 = 0;  // is beyond the usual 0.0 to 1.0 range
      ...
      if (y1 >= y2) {
        ...
        d->SRCADDR.reg =
            (uint32_t)eye[eyeNum].column[eye[eyeNum].colIdx].renderBuf +
            renderlen * 2;  // Point to END of data!
#else
        // Full column will be rendered; DISPLAY_SIZE pixels, point source to
        // end of renderBuf and enable source increment.
        d->BTCTRL.bit.SRCINC = 1;
        d->BTCNT.reg = DISPLAY_SIZE * 2;
        d->SRCADDR.reg =
            (uint32_t)eye[eyeNum].column[eye[eyeNum].colIdx].renderBuf +
        ...
        if (x < (DISPLAY_SIZE / 2)) {  // Left half of screen (quadrants 2, 3)
          ...
          displaceY =
              &displace[((DISPLAY_SIZE / 2 - 1) - x) * (DISPLAY_SIZE / 2)];
        ...
        for (; y <= y2; y++) {  // For each pixel of open eye in this column...
          ...
          if (y <
              (DISPLAY_SIZE / 2)) {  // Lower half of screen (quadrants 3, 4)
          ...
          if (dx < 255) {  // Inside eyeball area
            ...
            int mx = xx + dx * 2;  // Polar angle/dist map coords
            int my = yy + dy * 2;
            if ((mx >= 0) && (mx < mapDiameter) && (my >= 0) &&
                (my < mapDiameter)) {
              ...
              if (dist >= 0) {  // Sclera
                angle = ((angle + eye[eyeNum].sclera.angle) & 1023) ^
                        eye[eyeNum].sclera.mirror;
                int tx =
                    angle * eye[eyeNum].sclera.width / 1024;  // Texture map x/y
                int ty = dist * eye[eyeNum].sclera.height / 128;
                *ptr++ =
                    eye[eyeNum].sclera.data[ty * eye[eyeNum].sclera.width + tx];
              } else if (dist > -128) {  // Iris or pupil
                int ty = dist * iPupilFactor / -32768;
                if (ty >= eye[eyeNum].iris.height) {  // Pupil
                  *ptr++ = eye[eyeNum].pupilColor;
                } else {  // Iris
                  angle = ((angle + eye[eyeNum].iris.angle) & 1023) ^
                          eye[eyeNum].iris.mirror;
                  int tx = angle * eye[eyeNum].iris.width / 1024;
                  *ptr++ =
                      eye[eyeNum].iris.data[ty * eye[eyeNum].iris.width + tx];
                }
              } else {
  ...
  if (!x) {  // If it's the first column...
    ...
    eye[eyeNum].display->setAddrWindow(DISPLAY_X_OFFSET, DISPLAY_Y_OFFSET,
...
#define LIGHT_INTERVAL (1000000 / 10)  // 10 Hz, don't poll Seesaw too often
        if ((t - lastLightReadTime) >= LIGHT_INTERVAL) {
          ...
          if (rawReading <= 1023) {
            if (rawReading < lightSensorMin)
              rawReading = lightSensorMin;  // Clamp light sensor range
            else if (rawReading > lightSensorMax)
              rawReading = lightSensorMax;  // to within usable range
            float v = (float)(rawReading - lightSensorMin) /
                      (float)(lightSensorMax - lightSensorMin);  // 0.0 to 1.0
            v = pow(v, lightSensorCurve);
            lastLightValue = irisMin + v * irisRange;
            lastLightReadTime = t;
            lightSensorFailCount = 0;
          } else {  // I2C error
            if (++lightSensorFailCount >=
                25) {               // If repeated errors in succession...
              lightSensorPin = -1;  // Stop trying to use the light sensor
            } else {
              lastLightReadTime =
                  t - LIGHT_INTERVAL + 30000;  // Try again in 30 ms
            }
          }
        }
        irisValue =
            (irisValue * 0.97) +
            (lastLightValue * 0.03);  // Filter response for smooth reaction
      } else {
        ...
        for (uint16_t i = 0; i < IRIS_LEVELS; i++) {  // 0,1,2,3,...
          ...
          if (ibits) {
            ...
            iris_next[i] =
                -0.5 + ((float)random(1000) / 999.0);  // -0.5 to +0.5
          }
      ...
      if (voiceOn) {
        ...
        if (buttonState & (ARCADA_BUTTONMASK_UP | ARCADA_BUTTONMASK_A |
                           ARCADA_BUTTONMASK_DOWN)) {
          currentPitch = voicePitch(currentPitch);
          if (waveform) voiceMod(modulate, waveform);
  ...
  memcpy(eye[eyeNum].dptr,
         &eye[eyeNum].column[eye[eyeNum].colIdx].descriptor[0],
