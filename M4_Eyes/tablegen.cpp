...
void calcDisplacement() {
  ...
  if (displace = (uint8_t *)malloc((DISPLAY_SIZE / 2) * (DISPLAY_SIZE / 2))) {
    ...
    for (y = 0; y < 120; y++) {
      yield();  // Periodic yield() makes sure mass storage filesystem stays
                // alive
      dy = (float)y + 0.5;
      dy *= dy;  // Now dy^2
      for (x = 0; x < (DISPLAY_SIZE / 2); x++) {
        ...
        if (d2 <= eyeRadius2) {       // Pixel is within eye area
          ...
          *ptr++ = (uint8_t)(dx * pa) - x;  // Round to pixel space (no +0.5)
          // *ptr++ = 0;    // try not doing this thing.
...
void calcMap(void) {
  ...
  if (polarAngle =
          (uint8_t *)malloc(pixels * 2)) {      // Single alloc for both tables
      ...
      for (y = 0; y < mapRadius; y++) {
        ...
        for (x = 0; x < mapRadius; x++) {
          ...
          if (d2 <= irisRadius2) {  // If inside iris...
            ...
            for (int i = 126; i >= 0; i--) {
              float ratio = i / 128.0;  // 0.0 (open) to just-under-1.0 (slit)
                                        // (>= 1.0 will cause trouble)
              // Interpolate a point between top of iris and top of slit pupil,
              // based on ratio
              float y1 = iRad - (iRad - slitPupilRadius) * ratio;
              // (x1 is 0 and thus dropped from equation below)
              // And another point between right of iris and center of eye,
              // inverse ratio
              float x2 = iRad * (1.0 - ratio);
              // (y2 is also zero, same deal)
              // Find X coordinate of center of circle that crosses above two
              // points and has Y at 0.0
              ...
              if (d2 <= r2) {      // If point is within circle...
                polarDist[y * mapRadius + x] =
                    (int8_t)(-1 - i);  // Set to distance 'i'
                break;
              }
            }
          }
